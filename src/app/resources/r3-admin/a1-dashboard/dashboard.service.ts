...
export class DashboardService {
  ...
  async findStaticData(
    ...
    try {
      ...
      const totalSaleCurrent =
        (await Order.sum('total_price', currentPeriodFilter)) ?? 0;
      const totalSalePrevious =
        (await Order.sum('total_price', previousPeriodFilter)) ?? 0;
      const saleIncrease = totalSaleCurrent - totalSalePrevious;
      const saleDifferenceWithSign =
        saleIncrease >= 0 ? `+${saleIncrease}` : `${saleIncrease}`;
      ...
      if (totalSaleCurrent === 0 && totalSalePrevious === 0) {
        ...
        const percentageChange =
          ((totalSaleCurrent - totalSalePrevious) /
            (totalSaleCurrent + totalSalePrevious)) *
          100;
        totalPercentageIncrease = Math.max(
          -100,
          Math.min(percentageChange, 100),
        );
        totalPercentageIncrease = parseFloat(
...
async getStockStatus(): Promise<any> {
  ...
  async findCashierAndTotalSale(
    ...
    try {
      const { currentPeriodFilter, previousPeriodFilter } =
        this.getDateFilters(filters);
  ...
  private getDateFilters(filters: {
    today?: string;
    yesterday?: string;
    thisWeek?: string;
    thisMonth?: string;
  ...
  async findProductTypeWithProductHaveUsed(filters: {
    ...
    try {
      ...
      const { startDate, endDate } =
        this.getDateRange(filters) || this.getDefaultWeekRange();
    ...
    try {
      ...
      salesData.forEach((sale) => {
        ...
        if (!isNaN(saleDay.getTime())) {
          ...
          salesByDay[dayIndex].total_sales = parseFloat(
            sale.get('total_sales').toString(),
          );
        }
      });
  ...
  private getDateFilter(filters: {
    ...
    if (filters.today) {
      ...
      const startOfThreeMonthsAgo = this.startOfMonth(
        this.subtractMonths(new Date(), 3),
      );
      dateFilter['created_at'] = {
      ...
      const startOfSixMonthsAgo = this.startOfMonth(
        this.subtractMonths(new Date(), 6),
      );
      dateFilter['created_at'] = {
